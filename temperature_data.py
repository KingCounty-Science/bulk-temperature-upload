...
def import_from_folder():
    ...
    folder_path = Path(f"bulk_upload\{project}")
    ...
    for subfolder in subfolders:
        ...
        print(f"folder {subfolder.name}, site {site}, site sql id {site_sql_id}")
        ...
        for file in files:
            try:
                print("file: ",  file)
                #df = pd.read_csv(file, skiprows=1, usecols=[1,3]) # Adjust format as needed)  # Skip first row and ignore the first column , parse_dates=[0]
        ...
        os.makedirs(f"bulk_upload\{project}\{site}\metadata", exist_ok=True)
        # save file to metadata
        data.to_csv(f"bulk_upload\{project}\{site}\metadata\{site}_data.csv", index=False )
        # create a graph
        # Start a new plot
        ...
        plt.grid(False)
        plt.tight_layout()
        plt.savefig(f"bulk_upload\{project}\{site}\metadata\{site} water temperature.pdf")
        #plt.show()
...
def upload_data():
    ...
    for subfolder in subfolders:
        ...
        print(subfolder)
        site = subfolder.name
        data = pd.read_csv(f"bulk_upload\{project}\{site}\metadata\{site}_data.csv")
        data["datetime"] = pd.to_datetime(data["datetime"])
        print(data)
